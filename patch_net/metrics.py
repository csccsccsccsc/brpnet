...
def get_dice_1_multicls(true, pred, ths=0.0, eps=1e-7):
    """
        Traditional dice
    """
    ...
    for icls in range(1, ncls):
        ictrue = (true==icls).astype(np.float32)
        icpred = (pred[icls]>ths).astype(np.float32)
        inter = ictrue * icpred 
        denom = ictrue + icpred 
        dsc += (2.0 * np.sum(inter) + eps) / (np.sum(denom) + eps)
    dsc /= (ncls-1)
    return dsc
    

#####--------------------------As pseudocode
def get_dice_1(true, pred, ths=0.0):
    """
        Traditional dice
    """
    # cast to binary 1st
